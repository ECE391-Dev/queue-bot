...
async def get_questions_for_queue(base_url, queue_id, token=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Error fetching questions for queue {queue_id}: {response.status_code}")
            return []
...
def extract_netid(question):
    ...
    try:
        if question and 'askedBy' in question and question['askedBy'] and 'netid' in question['askedBy']:
...
def check_group_members_in_queue(netids, netid_to_group, group_to_members):
    ...
    return {group_id: members for group_id, members in groups_with_multiple_members.items() 
            if len(members) > 1}
...
async def check_queue_for_groups():
    ...
    if not queue_id:
        print("No queue ID specified. Set the DEFAULT_QUEUE_ID environment variable.")
        return
    
    # Get questions for the specified queue
    questions = await get_questions_for_queue(DEFAULT_BASE_URL, queue_id, QUEUE_TOKEN)
    
    if not questions:
        print(f"No questions found for queue {queue_id} or error fetching questions.")
        return
    ...
    # Check if multiple members of the same group are in the queue
    groups_in_queue = check_group_members_in_queue(netids, netid_to_group, group_to_members)
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        print("Error: No Discord token provided. Set the DISCORD_TOKEN environment variable.")
    elif not QUEUE_TOKEN:
        print("Error: No Queue token provided. Set the QUEUE_TOKEN environment variable.")
    else:
